...
function MusicUpdate()
	...
	if (felony<=0) and (stolen_car~=1) and (MusicRush<=0) and (Music1TargetVolume>0) then
		Music1TargetVolume=0;
		MusicTimeOut=5;
	end

	if (felony<=0) and (stolen_car~=1) and (MusicRush<=0) and (MusicTimeOut<=0) then
		local length=10;
	...
	if (Music1Volume>0) then
		alCmd(format("Gain music %f 0 1", (Music1Volume * music_volume)));
	end

	if (Music2Volume>0) then
		alCmd(format("Gain music %f 0 1", (Music2Volume * music_volume)));
	end
...
function WeatherUpdate()
	...
	if (CurrentRain>0) then
		...
		y=(y + 200);
		z=(z - 200);
		s=format("Move rain %d %d %d", x, y, z);
		Cmd(al, s);
		s=format("Move wind %d %d %d", x, y, z);
		Cmd(al, s);
	else
		local al=FindObject("OpenAL");
		local x, y, z=ScrPos();
		y=(y + 200);
		z=(z - 200);
		local s=format("Move wind %d %d %d", x, y, z);
		Cmd(al, s);
	end
	...
	if (CurrentRefl<TargetRefl) then
		...
		SetReflectivity(roadnetwork, CurrentRefl, CurrentRefl, CurrentRefl, CurrentRefl);
	end

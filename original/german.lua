...
function MissionInfo(misid, gamemode)
	misid=MissionRemap(misid);
	if (misid==3) then
		StartIntro("Sounds\\rap1.wav", "intro\\taxi", "intro\\slippy", nil, "Du startest das Spiel als Taxifahrer. Du musst in 15 Minuten 350$ verdienen. Je schneller du das Geld verdienst, desto besser dein Ergebnis. Vergiss nicht nachzutanken, bevor das dein Tank leer ist. Pass immer auf die Bullen auf!", 0, 0, 100);
	end
	...
	if (misid==7) then
		StartIntro("Sounds\\rap1.wav", "intro\\race", "intro\\slippy", nil, "Als du mit dem gerade gestohlenen Luxuswagen herumfuhrst, hast du die Chinesin in einem coolen Auto gesehen. Scheinbar will Sie ein Rennen gegen dich fahren. Du musst sie im Rennen schlagen, denn du musst auch noch unbedingt nach Chinatown.", 0, 0, 0);
	end

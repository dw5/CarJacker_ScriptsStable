...
function enter_security_code(code)
	...
	local w, h=ScrSize();
	local scale=(w / 1024);
	local tw=((10 * 20) * scale);
	local th=(30 * scale);
	local bw=(256 * scale);
	local bh=(64 * scale);
	rect(((w / 2) - (bw / 2)), ((h / 2) - (bh / 2)), bw, bh, 200);
	bw=(bw - (10 * scale));
	bh=(bh - (10 * scale));
	irect(((w / 2) - (bw / 2)), ((h / 2) - (bh / 2)), bw, bh, 200);
	bw=(bw - (20 * scale));
	bh=(bh - (15 * scale));
	ScrRect("", ((w / 2) - (bw / 2)), ((h / 2) - (bh / 2)), bw, bh, BLEND, 100, 100, 80, 255);
	scannertext(dispcode, (w / 2), (((h / 2) - (bh / 2)) + (4 * scale)), scale, scale);
...
function display_scanner(code)
	...
	bw=(bw - (20 * scale));
	bh=(bh - (20 * scale));
	irect(((w / 2) - (bw / 2)), ((h / 2) - (bh / 2)), bw, (bh - 32), 200);
	bw=(bw - (20 * scale));
	bh=(bh - (16 * scale));
	ScrRect("", ((w / 2) - (bw / 2)), ((h / 2) - (bh / 2)), bw, (bh - 32), BLEND, 100, 100, 80, 255);
	ScrRect("GUI\\scanner", ((w / 2) - ((128 * scale) / 2)), ((((h / 2) - (bh / 2)) + bh) - (32 * scale)), (128 * scale), (48 * scale), BLEND, 255, 255, 255, 255);
	local y=((h / 2) - (bh / 2));
	local p="< Scanning";
	for i=0, 3 do
		if (i<=floor(mod((GetTime() * 4), 3))) then
			p=p .. ".";
		else
			p=p .. " ";
		end
	...
	p=p .. ">";
	scannertext(p, (w / 2), y, (scale * 0.7), scale);
	y=(y + (35 * scale));
	local t=format("%03d.%03d Mhz %03d.%03d Mhz", random(0, 999), random(0, 999), random(0, 999), random(0, 999));
	scannertext(t, (w / 2), y, (scale * 0.5), scale);
	y=(y + (25 * scale));
	t=format("%03d.%03d Mhz %03d.%03d Mhz", random(0, 999), random(0, 999), random(0, 999), random(0, 999));
	scannertext(t, (w / 2), y, (scale * 0.5), scale);
	y=(y + (25 * scale));
	t=format("%03d.%03d Mhz %03d.%03d Mhz", random(0, 999), random(0, 999), random(0, 999), random(0, 999));
	scannertext(t, (w / 2), y, (scale * 0.5), scale);
	...
	scannertext(t, (w / 2), y, (scale * 0.5), scale);
	y=(y + (30 * scale));
	scannertext(code, (w / 2), y, scale, scale);
end
...
function progressbox(title, progress, orientation, r, alpha)
	...
	local w, h=ScrSize();
	local scale=(w / 1024);
	local tw=((strlen(title) * 20) * scale);
	local th=((30 * scale) * 2);
	local bw=(tw + 48);
	local bh=(th + 48);
	local y=(h / 2);
...
function gui_RenderRaceMsg()
	...
	local x=((w / 2) - ((strlen(gui_raceMsg) * cw) / 2));
	local y=((h / 2) - (ch / 2));
...
function gui_DisplayHelpMsg()
	...
	for i=1, gui_NumMessages do
		local tw=(strlen(gui_HelpMsg[i]["title"]) * CW);
		if (tw>wbox) then
			wbox=tw;
		end
	...
	for i=1, gui_NumMessages do
		...
		ScrRect("#" .. gui_HelpMsg[i]["title"], x, y, CW, CH, BLEND, gui_HelpMsg[i]["tr"], gui_HelpMsg[i]["tg"], gui_HelpMsg[i]["tb"], 255);
		y=(y + CH);
	end
...
function gui_DisplayMessage()
	...
	local LEAD=(3 * s);
	if (msg_line1) then
		if ((strlen(msg_line1) * CW)>textw) then
...
function gamegui_Update()
	...
	if (IsPaused()>0) then
		...
		local scale=(w / 1024);
		local strsize=(((strlen(str) - 1) * 25) * scale);
		local x=((w / 2) - (strsize / 2));
		local y=((h / 2) - (20 * scale));
		ScrRect(str, (x + 2), (y + 2), (25 * scale), (40 * scale), BLEND, 0, 0, 0, 128);
		ScrRect(str, x, y, (25 * scale), (40 * scale), BLEND, 255, 255, 255, 255);
		if (LastKeyDown==89) then
			guit2menu();
			StartIntro();
		end

		if (LastKeyDown==78) then
			Resume();
		end
